...
public class LargeEssentiaEnergyData {
    ...
    public static String readJsonFile() {
        try {
            URL url = Thread.currentThread().getContextClassLoader()
                    .getResource("assets/largeessentiagenerator/data/essentia.json");
            assert url != null;
    ...
    public static void processEssentiaData() {
        ...
        for (JsonElement elm : jsonArray) {
            ...
            if (aspect != null) {
                int fuel = essData.get("fuelValue").getAsInt()*64/250;
    ...
    public static int getAspectFuelValue(Aspect aspect) {
        if (ASPECT_FUEL_DATA.containsKey(aspect)) {
            return (int) (ASPECT_FUEL_DATA.get(aspect).getFuelValue() * C);
        } else return 0;
    }
